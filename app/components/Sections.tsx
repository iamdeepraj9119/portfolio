...
import React from "react"; import Image from "next/image"; import { motion } from "framer-motion"; import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer } from "recharts"; import CertificateGallery from "./CertificateGallery";
...
{/* NAVBAR */}
  ...
  <div className="px-6 md:px-20 pt-32 pb-20 space-y-32">
    ...
    {/* ABOUT */}
    <motion.section id="about" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} className="max-w-3xl">
      <h2 className="text-4xl font-bold mb-4 border-b border-gray-700 pb-2">About Me</h2>
      <p className="text-gray-400">
        I am a passionate Data Analyst skilled in Python, SQL, Excel, and Power BI. I focus on solving real-world business problems using data.
      </p>
    </motion.section>
    ...
    <motion.section id="projects" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }}>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {[
          { title: "HR Analytics", desc: "Employee insights dashboard", link: "#" },
          { title: "Sales Dashboard", desc: "Power BI sales insights", link: "#" },
          { title: "Python Analysis", desc: "Data cleaning + visualization", link: "#" }
        ].map((p) => (
          <div key={p.title} className="p-6 bg-white/10 rounded-2xl hover:scale-105 transition">
            ...
            <a href={p.link} className="text-blue-400 text-sm">View Project</a>
          </div>
        ))}
      </div>
    </motion.section>
    ...
    {/* CONTACT */}
    <motion.section id="contact" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} className="text-center">
      ...
      <a href="mailto:your@email.com" className="px-6 py-2 bg-blue-500 rounded-xl">Email Me</a>
    </motion.section>
